...
class JiraBinder(Binder):
    """ Generic Binder for Jira """


@jira
class JiraModelBinder(JiraBinder):
    """
    Bindings are done directly on the binding model.

    Binding models are models called ``jira.{normal_model}``,
    like ``jira.res.partner`` or ``jira.product.product``.
    They are ``_inherits`` of the normal models and contains
    the Jira ID, the ID of the Jira Backend and the additional
    fields belonging to the Jira instance.
    """
    _model_name = [
        'jira.project',
    ]
    ...
    def to_backend(self, record_id, wrap=False):
        ...
        if wrap:
            ...
            if binding:
                binding.ensure_one()
                return binding.jira_id
            else:
                return None
        if not record:
            record = self.model.browse(record_id)
        assert record
        return record.jira_id
    ...
    def bind(self, external_id, binding_id):
        ...
        :type binding_id: int
        """
        # the external ID can be 0 on Jira! Prevent False values
        ...
        # avoid to trigger the export when we modify the `jira_id`
    ...
    def unwrap_binding(self, binding_id, browse=False):
        ...
        Example: when called with a ``jira.product.product`` id,
    ...
    def unwrap_model(self):
        ...
        Example: when called on a binder for ``jira.product.product``,
