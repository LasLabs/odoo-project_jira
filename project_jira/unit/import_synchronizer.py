...
class JiraImporter(Importer):
    ...
    def __init__(self, connector_env):
        ...
        """
        super(JiraImporter, self).__init__(connector_env)
        self.jira_id = None
        self.jira_record = None

    def _get_jira_data(self):
        """ Return the raw Jira data for ``self.jira_id`` """
        return self.backend_adapter.read(self.jira_id)
    ...
    def _is_uptodate(self, binding):
        ...
        assert self.jira_record
        if not self.jira_record.get('updated_at'):
        ...
        jira_date = from_string(self.jira_record['updated_at'])
        # if the last synchronization date is greater than the last
        # update in jira, we skip the import.
        # Important: at the beginning of the exporters flows, we have to
        # check if the jira_date is more recent than the sync_date
        # and if so, schedule a new import. If we don't do that, we'll
        # miss changes done in Jira
        return jira_date < sync_date

    def _import_dependency(self, jira_id, binding_model,
                           importer_class=None, always=False):
        """ Import a dependency.

        The importer class is a class or subclass of
        :class:`JiraImporter`. A specific class can be defined.

        :param jira_id: id of the related binding to import
        ...
        :param always: if True, the record is updated even if it already
                       ...
                       not been modified on Jira since the last
        ...
        if always or binder.to_openerp(jira_id) is None:
            importer = self.unit_for(importer_class, model=binding_model)
            importer.run(jira_id)
    ...
    def _map_data(self):
        ...
        """
        return self.mapper.map_record(self.jira_record)
    ...
    def _get_binding(self):
        return self.binder.to_openerp(self.jira_id, browse=True)
    ...
    def _create(self, data):
        ...
        _logger.debug('%d created from jira %s', binding, self.jira_id)
        return binding
    ...
    def _update(self, binding, data):
        ...
        _logger.debug('%d updated from jira %s', binding, self.jira_id)
        return
    ...
    def run(self, jira_id, force=False):
        """ Run the synchronization

        :param jira_id: identifier of the record on Jira
        """
        self.jira_id = jira_id
        try:
            self.jira_record = self._get_jira_data()
        ...
        self.binder.bind(self.jira_id, binding)

        self._after_import(binding)


JiraImportSynchronizer = JiraImporter  # deprecated
...
@jira
class SimpleRecordImporter(JiraImporter):
    """ Import one Jira Website """
    _model_name = [
        'jira.website',
...
class TranslationImporter(Importer):
    ...
    _model_name = ['jira.product.category',
                   'jira.product.product',
                   ]

    def _get_jira_data(self, storeview_id=None):
        """ Return the raw Jira data for ``self.jira_id`` """
        return self.backend_adapter.read(self.jira_id, storeview_id)

    def run(self, jira_id, binding_id, mapper_class=None):
        self.jira_id = jira_id
        storeviews = self.env['jira.storeview'].search(
        ...
        for storeview in lang_storeviews:
            lang_record = self._get_jira_data(storeview.jira_id)
...
class AddCheckpoint(ConnectorUnit):
    ...
    (not the jira.* but the _inherits'ed model) """

    _model_name = ['jira.product.product',
                   'jira.product.category',
                   ]
...
def import_record(session, model_name, backend_id, jira_id, force=False):
    """ Import a record from Jira """
    env = get_environment(session, model_name, backend_id)
    importer = env.get_connector_unit(JiraImporter)
    importer.run(jira_id, force=force)
