...
from .backend_adapter import JIRA_DATETIME_FORMAT
...
class JiraBaseExporter(Exporter):
    ...
    def __init__(self, connector_env):
        ...
        """
        super(JiraBaseExporter, self).__init__(connector_env)
    ...
    def _delay_import(self):
        ...
        import_record.delay(self.session, self.model._name,
                            self.backend_record.id, self.jira_id,
                            force=True)
    ...
    def run(self, binding_id, *args, **kwargs):
        ...
        self.jira_id = self.binder.to_backend(self.binding_id)
        try:
        ...
        self.binder.bind(self.jira_id, self.binding_id)
    ...
    def _after_export(self):
        """ Can do several actions after exporting a record on jira """
        pass
...
class JiraExporter(JiraBaseExporter):
    ...
    def __init__(self, connector_env):
        ...
        :type connector_env: :class:`connector.connector.ConnectorEnvironment`
        """
        super(JiraExporter, self).__init__(connector_env)
    ...
    def _retry_unique_violation(self):
        ...
        record created by :meth:`_export_dependency`), resulting in:
            ...
            constraint "jira_product_product_openerp_uniq"
    ...
    def _export_dependency(self, relation, binding_model, exporter_class=None,
        ...
        if wrap and hasattr(relation, binding_field):
            ...
            # jira.product.product and we are exporting it, but we need to
    ...
    def _update(self, data):
        """ Update an Jira record """
        assert self.jira_id
        # special check on data before export
        self._validate_update_data(data)
        self.backend_adapter.write(self.jira_id, data)
    ...
    def _run(self, fields=None):
        ...
        if self.jira_id:
            ...
            if not record:
                return _('Nothing to export.')
            self.jira_id = self._create(record)
        return _('Record exported with ID %s on Jira.') % self.jira_id
...
def export_record(session, model_name, binding_id, fields=None):
    ...
    exporter = env.get_connector_unit(JiraExporter)
